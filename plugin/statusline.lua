...
local icons, lsp, highlight, decorations = mw.styles.icons, mw.styles.lsp, mw.highlight, mw.styles.decorations
local api, fn, fs, fmt, strwidth = vim.api, vim.fn, vim.fs, string.format, vim.api.nvim_strwidth
local P, falsy = mw.styles.palette, mw.falsy
...
local function diagnostic_info(context)
  ...
  local lsp_icons = mw.styles.icons.lsp
  local result = {
  ...
  return mw.fold(function(accum, item)
...
function mw.styles.statusline.lsp_client_click()
...
---@return string
function mw.styles.statusline.render()
  ...
  local lsp_clients = mw.map(function(client)
    return {
      {
      ...
      click = "v:lua.mw.styles.statusline.lsp_client_click",
...
-- set the statusline
vim.o.statusline = "%{%v:lua.mw.styles.statusline.render()%}"

mw.augroup("CustomStatusline", {
  event = "FocusGained",
  command = function()
  ...
  command = function(args)
    ...
    if #clients > MAX_LSP_SERVER_COUNT then
