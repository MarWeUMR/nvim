...
mw.styles.lsp = {
  colors = {
    error = mw.styles.palette.pale_red,
    warn = mw.styles.palette.dark_orange,
    hint = mw.styles.palette.bright_blue,
    info = mw.styles.palette.teal,
  },
...
local filenames = mw.p_table {
  ["option-window"] = presets.tool_panel,
}

mw.styles.decorations = {}

---@alias styles.OptionValue (boolean | string)

---Get the mw.styles setting for a particular filetype
...
function mw.styles.decorations.get(opts)
...
function mw.styles.decorations.set_colorcolumn(bufnr, fn)
  local buf = vim.bo[bufnr]
  local decor = mw.styles.decorations.get { ft = buf.ft, bt = buf.bt, setting = "colorcolumn" }
  if buf.ft == "" or buf.bt ~= "" or decor.ft == false or decor.bt == false then
    return
  end
  local ccol = decor.ft or decor.bt or ""
  local virtcolumn = not mw.falsy(ccol) and ccol or "+1"
...
----------------------------------------------------------------------------------------------------
mw.styles.current = { border = mw.styles.border.rectangle }
