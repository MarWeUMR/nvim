...
---@param list T[]
---@return T[]
function core.map(callback, list)
...
---@param list string[]
---@return boolean
function core.any(target, list)
  return core.fold(function(accum, item)
...
core.list_installed_plugins = (function()
  local plugins
  return function()
...
---@return boolean
function core.plugin_installed(plugin_name)
  for _, path in ipairs(core.list_installed_plugins()) do
...
---@param plugin_name string
---@return boolean?
function core.plugin_loaded(plugin_name)
...
---@return boolean
function core.is_vim_list_open()
...
---@return string
function core.truncate(str, max_len)
  assert(str and max_len, 'string and max_len must be provided')
  return api.nvim_strwidth(str) > max_len and str:sub(1, max_len) .. core.style.icons.misc.ellipsis
    or str
end
...
function core.ftplugin_conf(name, callback)
  local plugin_name = type(name) == 'table' and name.plugin or nil
  if plugin_name and not core.plugin_loaded(plugin_name) then return end

  local module = type(name) == 'table' and name[1] or name
  local info = debug.getinfo(1, 'S')
  local ok, plugin = core.require(module, { message = fmt('In file: %s', info.source) })
...
---@return unknown
function core.version(major, minor, _)
...
---@param opts table?
function core.command(name, rhs, opts)
  opts = opts or {}
...
---@param str string
---@return string
function core.replace_termcodes(str) return api.nvim_replace_termcodes(str, true, true, true) end
